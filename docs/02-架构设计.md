...
do { begin_res = co_await begin_aw; } while (begin_res && !begin_res->has_value());
if (!begin_res) { /* 错误处理 */ co_return; }

for (auto& item : items) {
    auto q_aw = client.query("INSERT INTO ...");
    std::expected<std::optional<MysqlResultSet>, MysqlError> q_res;
    do { q_res = co_await q_aw; } while (q_res && !q_res->has_value());
    if (!q_res) { /* 错误处理 */ co_return; }
}

auto commit_aw = client.commit();
std::expected<std::optional<MysqlResultSet>, MysqlError> commit_res;
do { commit_res = co_await commit_aw; } while (commit_res && !commit_res->has_value());
if (!commit_res) { /* 错误处理 */ co_return; }
```
...
```cpp
auto prep_aw = client.prepare("SELECT * FROM users WHERE id = ?");
std::expected<std::optional<MysqlPrepareAwaitable::PrepareResult>, MysqlError> prep_res;
do { prep_res = co_await prep_aw; } while (prep_res && !prep_res->has_value());
if (!prep_res) { /* 错误处理 */ co_return; }

uint32_t stmt_id = prep_res->value().statement_id;
for (auto id : ids) {
    std::vector<std::optional<std::string>> params = {std::to_string(id)};
    auto exec_aw = client.stmtExecute(stmt_id, params);
    std::expected<std::optional<MysqlResultSet>, MysqlError> exec_res;
    do { exec_res = co_await exec_aw; } while (exec_res && !exec_res->has_value());
    if (!exec_res) { /* 错误处理 */ co_return; }
}
```
