...
3. 是否选择了正确的数据库：
   ```cpp
   if (auto use_db = co_await client.useDatabase("correct_db"); !use_db) {
       std::cerr << "切库失败\n";
   }
   ```
...
```cpp
if (auto __await_result = co_await client.query("SET NAMES utf8mb4"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
if (auto __await_result = co_await client.stmtExecute(stmt_id, params, param_types); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
for (const auto& name : names) {
    if (auto __await_result = co_await client.stmtExecute(stmt_id, {name}); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
```
