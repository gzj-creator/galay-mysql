...
pool_cfg.mysql_config = config;
pool_cfg.async_config = async_config;
...
if (auto __await_result = co_await client.stmtExecute(stmt_id, params); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
if (auto __await_result = co_await client.stmtExecute(stmt_id, std::span(params)); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
// 低效：每次自动提交
for (auto& item : items) {
    if (auto __await_result = co_await client.query("INSERT INTO ..."); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}

// 高效：批量提交
if (auto __await_result = co_await client.beginTransaction(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
for (auto& item : items) {
    if (auto __await_result = co_await client.query("INSERT INTO ..."); !__await_result) {
...
if (auto __await_result = co_await client.commit(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
// 低效：每次解析 SQL
for (auto id : ids) {
    if (auto __await_result = co_await client.query("SELECT * FROM users WHERE id = " + std::to_string(id)); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
...
for (auto id : ids) {
    if (auto __await_result = co_await client.stmtExecute(stmt_id, {std::to_string(id)}); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
```
...
try {
    if (auto __await_result = co_await client.query("UPDATE accounts SET balance = balance - 100 WHERE id = 1"); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    if (auto __await_result = co_await client.query("UPDATE accounts SET balance = balance + 100 WHERE id = 2"); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    if (auto __await_result = co_await client.commit(); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    ...
    if (auto __await_result = co_await client.rollback(); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    throw;
}
...
```cpp
// 设置为 READ COMMITTED
if (auto __await_result = co_await client.query("SET TRANSACTION ISOLATION LEVEL READ COMMITTED"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await client.beginTransaction(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
...
if (auto __await_result = co_await client.query("INSERT INTO users ..."); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await client.query("SAVEPOINT sp1"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await client.query("INSERT INTO logs ..."); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
// 回滚到保存点
if (auto __await_result = co_await client.query("ROLLBACK TO SAVEPOINT sp1"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await client.commit(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
...
```cpp
// 定期 ping 保持连接活跃
if (auto __await_result = co_await client.ping(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
```cpp
if (auto __await_result = co_await client.useDatabase("another_db"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
if (auto __await_result = co_await client.query("SELECT * FROM users WHERE name = '" + user_input + "'"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// 安全：使用预处理语句
auto prep = co_await client.prepare("SELECT * FROM users WHERE name = ?");
if (auto __await_result = co_await client.stmtExecute(prep->value().statement_id, {user_input}); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
if (auto __await_result = co_await client->query("..."); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
// 连接泄漏！
